...
GLFWwindow * glfwStartUp(int& argCount, char* argValues[],
	string windowTitle = "Project 3", int width = 1000, int height = 1000);
...
void displayDirectional() {
	...
	//do not need to draw the large center cow
	//glDrawArrays(GL_TRIANGLES, 0, nbrTriangles * 3);

	mat4x4 scale;
	mat4x4 translate;
	mat4x4_identity(scale);
	mat4x4_scale_aniso(scale, scale, 1.5f, 1.5f, 1.5f);;
	mat4x4 modelMatrix;
...
int main(int argCount, char* argValues[]) {
	...
	srand(time(NULL));
	window = glfwStartUp(argCount, argValues, "Particle System - Collin Creps");
